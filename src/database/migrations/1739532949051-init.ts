...
export class Init1739532949051 implements MigrationInterface {
  name = 'Init1739532949051';
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `CREATE TABLE \`call\` (\`id\` varchar(36) NOT NULL, \`created_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`deleted_at\` datetime(6) NULL, \`name\` varchar(255) NOT NULL, \`description\` text NOT NULL, \`published_at\` datetime NULL, \`cover\` varchar(255) NULL, \`document\` varchar(255) NULL, \`form\` json NULL, \`reviewers\` json NULL, \`requirements\` json NULL, \`authorId\` varchar(36) NULL, \`publisherId\` varchar(36) NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `ALTER TABLE \`call\` ADD CONSTRAINT \`FK_e351ed7848e2b0c5d0d6c570dd9\` FOREIGN KEY (\`authorId\`) REFERENCES \`user\`(\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`
    );
    await queryRunner.query(
      `ALTER TABLE \`call\` ADD CONSTRAINT \`FK_2739d752e899e1921e4aa3d7f04\` FOREIGN KEY (\`publisherId\`) REFERENCES \`user\`(\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`
    );
    await queryRunner.query(
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(`ALTER TABLE \`call\` DROP FOREIGN KEY \`FK_2739d752e899e1921e4aa3d7f04\``);
    await queryRunner.query(`ALTER TABLE \`call\` DROP FOREIGN KEY \`FK_e351ed7848e2b0c5d0d6c570dd9\``);
